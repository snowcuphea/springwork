 ...
 * => 미리 요청 path별로 실행할 실제 클래스를 등로갈 설정파일을 보고 객체를 찾아서 생성
 *	  mycommand.peroperties파일에 등록된 요청path를 보고 실행할 객체를 찾는다.
...
public class RequestMapping {
	...
	public Action mapping(String path) {
		...
		// 실제는 xml문서나 properties 파일을 분석해서 자동으로 만들도록 코드를 정의할 것이므로, 코드 변경을 하지 않아도 된다. 
